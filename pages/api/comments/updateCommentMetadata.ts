...
const updateCommentMetadata = async(commentId: string, deleted: string, edited: string, flagged: string, user: UserProfile) => {
...
export default withApiAuthRequired(async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    return validateQueryData(req.query, 'updateCommentMetadata') ? res.status(200).send(await updateCommentMetadata(req.query.id as string, req.query.deleted as string, req.query.edited as string, req.query.flagged as string, session.user)) : res.status(400).send({"error": "Invalid update comment metadata"})
  } catch (error) {
