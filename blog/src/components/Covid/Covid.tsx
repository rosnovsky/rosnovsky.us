...
export default function Covid() {
  ...
  useEffect(() => {
    const fetchCases = async () => {
      const currentTotals = await fetch(
  ...
  return (
    <div className="xs:mx-5 lg:w-4/5 mx-auto sm:mx-5 md:mx-5 mb-10 xl:w-2/3">
      <h3 className="text-lg leading-6 font-medium text-gray-900 mt-10">
        COVID-19: US Stats
      </h3>
      {/* TODO: Turn each card into its own component */}
      <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 border-orange-900">
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-5 md:mx-auto sm:mx-auto w-0 flex-1">
                <dl>
                  <dt className="text-sm leading-5 font-medium text-gray-500 truncate">
                    Total Cases
                  </dt>
                  <dd className="flex items-baseline">
                    <div className="text-2xl sm:text-xl md:text-xl leading-6 font-semibold text-gray-900">
                      {cases
                        ? `${(cases[0].positive / 1000000).toFixed(1)} MM`
                        : 'Trump Lied'}
                    </div>
                    <div className="ml-1 flex items-baseline text-sm leading-5 font-semibold text-green-600">
                      <svg
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1 md:mx-auto sm:mx-auto">
                <dl>
                  <dt className="md:hidden sm:hidden text-sm leading-5 font-medium text-gray-500 truncate">
                    Currently Hospitalized
                  </dt>
                  <dt className="xs:hidden xl:hidden lg:hidden text-sm leading-5 font-medium text-gray-500">
                    Hospitalized
                  </dt>
                  <dd className="flex items-baseline">
                    <div className="text-2xl leading-8 font-semibold text-gray-900">
                      {cases &&
                        (cases[0].hospitalizedCurrently / 1000).toFixed(2)}
                      K
                    </div>
                    <div className="ml-2 flex items-baseline text-sm leading-5 font-semibold text-green-600">
                      <svg
                      ...
                      <span className="sr-only">Increased by</span>
                      {cases &&
                        (cases[0].hospitalizedIncrease / 1000).toFixed(2)}
                      K
                    </div>
                  </dd>
                </dl>
