...
export async function GenerateSocialCards(posts: Post[]) {
  posts.map(post => {
    ...
    const fetchUrl = fetch(
      `https://rosnovsky.us/api/generateOgImage?title=${encodeURIComponent(socialTitle)}&date=${format(
        Date.parse(publishedAt),
        'dd MMM yyyy'
      )}&category=${
        categories[0].title
      }&subtitle=${encodeURIComponent(socialSubtitle)}&coverImage=${encodeURIComponent(
        mainImage?.asset.url
      )}`,
      {
        mode: 'cors',
