...
- Whatever tool you're building, you have to **think about __how__ it will be used to abuse others** (not __if__ it will be used for abuse). You must figure out a balance between privacy and safety, and it's hell of a task.
- **NPM and the entire JS ecosystem is wild**: this site, for example, relies on thousands of packages created by thousands of people, and I have to trust the entire supply chain. We had issues with this sort of thing [in the past](https://thehackernews.com/2021/10/malicious-npm-libraries-caught.html), and supply chain attacks are not going away. There are tools that can help, though, and tracking simple things (like typos in package names) is a good start.
