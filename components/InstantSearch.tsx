...
export const InstantSearch = ({ posts }: { posts: BlogPost[] }) => {
  ...
  return (<><div className="w-full mb-10 "><input className="bg-white focus:outline-none focus:shadow-outline border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-4 block w-full appearance-none text-black leading-normal" type="text" placeholder="Instant search" onChange={(e) => setSearchTerm(e.target.value)} /><div className="text-xs text-right text-black mt-1 dark:text-white">ℹ <Link href="/about" passHref><span className="hover:underline text-black cursor-point dark:text-white">How it works</span></Link></div></div>
    <div className="relative w-full mb-4">
      ...
      {process.env.NODE_ENV !== 'test' ? <InView className="text-black text-center mx-auto" as="div" onChange={(inView) => inView ? setPage(page + 1) : null}>
        {!searchTerm ? <Image src={require('../public/static/images/backend/error.png')} alt="The End Of The Internet" /> : searchResults.length < 1 ? "Nothing found" : null}
      </InView> : !searchTerm ? <Image src='/public/static/images/backend/error.png' alt="The End Of The Internet" layout='fill' /> : searchResults.length < 1 ? "Nothing found" : null}
    </div></>)
}
