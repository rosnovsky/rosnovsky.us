...
export default function Comment({ postComment, postId, postTitle }: { postComment: PostComment, postId: string, postTitle: string }) {
  const [userProfile, setUserProfile] = useState<UserProfile>()

  useEffect(() => {
    const fetchUserProfile = async () => {
      return fetch(process.env.NODE_ENV == "production" ? `https://rosnovsky.us/api/comments/userProfile?user_id=${postComment.user_id}` : `http://localhost:3000/api/comments/userProfile?user_id=${postComment.user_id}`).then(result => result.json()).then((user) => setUserProfile(user.user))
    };
    fetchUserProfile();
  }, []);
  ...
  const deleteComment = () => {
    fetch(`/api/comments/updateComment`, {
      method: 'POST',
      body: JSON.stringify({
        id: postComment.id, content: postComment.comment, postId, postTitle, operation: 'delete'
      })
    })
    return undefined
  }
  ...
  const flagComment = () => {
    fetch(`/api/comments/updateComment`, {
      method: 'POST',
      body: JSON.stringify({
        id: postComment.id, content: postComment.comment, postId, postTitle, operation: 'flag'
      })
    })
    return undefined
  }
  ...
  return (
    <div className="my-10">
      <div className="w-full p-3 flex items-center justify-center">
        <div className={postComment.flagged ? "bg-gray-200 border border-red-800 dark:bg-gray-700  shadow-sm mx-auto px-4 py-3 rounded-lg w-full" : "bg-white dark:bg-gray-900 border shadow-sm mx-auto px-4 py-3 rounded-lg w-full"}>
          <div className="flex items-center">
            <img className="h-12 w-12 rounded-full" alt={`${userProfile?.nickname}'s avatar`} src={userProfile?.picture!} />
            <div className="ml-2">
              <div className="text-sm ">
                <span className="font-semibold">
                  {userProfile ? userProfile.nickname!.charAt(0).toUpperCase() + userProfile?.nickname?.slice(1) : "loading..."} {postComment.user_id === "auth0|60f1f34374a38b006885a17d" ?
                    <span>
                ...
                <span className="text-gray-500 dark:text-gray-700"> • {new Date(postComment.published_at).toLocaleString('en-US', { day: "numeric", month: "long", year: "numeric" })}</span>
              </div>
              <div className="text-gray-500 dark:text-gray-700 text-xs flex flex-row">
                <span className="inline-block">{postComment.edited ?
          ...
          <p className="text-gray-800 dark:text-gray-200 text-sm mt-2 leading-normal md:leading-relaxed">{postComment.comment}</p>
          <div className="text-gray-500 dark:text-gray-700 flex items-center mt-3">
            {user?.sub === postComment.user_id ? <button onClick={deleteComment}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="16"><path d="M32 464a48 48 0 0 0 48 48h288a48 48 0 0 0 48-48V96H32zm272-288a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zm-96 0a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zm-96 0a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0z" fill="pink" /><path d="M432 32H312l-9.4-18.7A24 24 0 0 0 281.1 0H166.8a23.72 23.72 0 0 0-21.4 13.3L136 32H16A16 16 0 0 0 0 48v32a16 16 0 0 0 16 16h416a16 16 0 0 0 16-16V48a16 16 0 0 0-16-16zM128 160a16 16 0 0 0-16 16v224a16 16 0 0 0 32 0V176a16 16 0 0 0-16-16zm96 0a16 16 0 0 0-16 16v224a16 16 0 0 0 32 0V176a16 16 0 0 0-16-16zm96 0a16 16 0 0 0-16 16v224a16 16 0 0 0 32 0V176a16 16 0 0 0-16-16z" fill="red" /></svg>&nbsp;&nbsp;</button> : null}
            {user?.sub !== postComment.user_id ? <span>
              {
                postComment.flagged
                  ...
                  : user?.sub
                    ? <button onClick={flagComment}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="16"><path d="M512 277.9c-.16 30.35-26.16 58.1-59.55 58.1H350.72C358 364.34 384 388.13 384 430.55 384 504 336 512 312 512c-20.18 0-29.48-39.29-33.93-57.79-5.21-21.67-10.59-44.07-25.39-58.91-32.47-32.52-49.51-74-89.12-113.11a12 12 0 0 1-3.56-8.52V59.9a12 12 0 0 1 11.78-12c15.83-.29 36.7-9.08 52.66-16.17C256.19 17.6 295.71 0 344 0h2.85c42.78 0 93.36.41 113.77 29.74 8.39 12.05 10.45 27 6.15 44.63 16.31 17 25.06 48.86 16.38 74.76 17.55 23.43 19.15 56.13 9.31 79.46l.11.12C504.45 240.65 512.08 260 512 277.9z" fill="gray" /><path d="M104 32H24A24 24 0 0 0 0 56v240a24 24 0 0 0 24 24h80a24 24 0 0 0 24-24V56a24 24 0 0 0-24-24zM64 280a24 24 0 1 1 24-24 24 24 0 0 1-24 24z" fill="lightgray" /></svg></button>
                    : null}</span>
              : null}
          </div>
        </div>
