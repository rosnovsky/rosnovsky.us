    ...
    const fetchUserProfile = async () => {
      return fetch(
        process.env.NODE_ENV == 'production'
          ? `https://rosnovsky.us/api/comments/userProfile?user_id=${postComment.user_id}`
          : `http://localhost:3000/api/comments/userProfile?user_id=${postComment.user_id}`
      )
    ...
    <div className="my-10">
      <div className="w-full p-3 flex items-center justify-center">
        <div
          className={
            postComment.flagged
              ? 'bg-gray-200 border border-red-800 dark:bg-gray-700  shadow-sm mx-auto px-4 py-3 rounded-lg w-full'
              : 'bg-white dark:bg-gray-900 border shadow-sm mx-auto px-4 py-3 rounded-lg w-full'
          }
        >
                ...
                <span className="font-semibold">
                  {userProfile?.nickname
                    ? userProfile.nickname.charAt(0).toUpperCase() +
                      userProfile.nickname.slice(1)
                    : 'loading...'}{' '}
                  {postComment.user_id === 'auth0|60f1f34374a38b006885a17d' ? (
                    <span>
                      <svg
          ...
          <p className="text-gray-800 dark:text-gray-200 text-sm mt-2 leading-normal md:leading-relaxed">
            {postComment.comment}
          </p>
          <div className="text-gray-500 dark:text-gray-700 flex items-center mt-3">
            {user?.sub === postComment.user_id ? (
